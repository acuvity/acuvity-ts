...
Apex.prototype.scan = async function ({ messages, files, requestType, annotations, analyzers, bypassHash, anonymization, redactions, keywords, accessPolicy, contentPolicy, }) {
    const request = {};
    if (messages) {
...
Apex.prototype.police = async function ({ messages, files, requestType, annotations, bypassHash, anonymization, provider, user, }) {
    const request = {};
    if (messages) {
        if (typeof messages === "string") {
            request.messages = [messages];
...
Apex.prototype.listAnalyzers = async function () {
    return (this._available_analyzers ?? (this._available_analyzers = await this.listAnalyzers()));
};
Apex.prototype.listAnalyzerNames = async function (group) {
...
Apex.prototype.listAnalyzerGroups = async function () {
    const analyzers = (this._available_analyzers ?? (this._available_analyzers = await this.listAnalyzers()));
    return analyzers
        .map((a) => a.group ?? "")
        .filter((a) => a !== "")
